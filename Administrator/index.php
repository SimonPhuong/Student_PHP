...
if(isset($_POST['login']))
  {
    ...
    $stmt = $con->prepare("SELECT AdminUserName, AdminPassword FROM tkadmin WHERE AdminUserName = :username");
    $stmt->bindParam(':username', $uname);
    $stmt->execute();
    $row = $stmt->fetch(PDO::FETCH_ASSOC);

    if($row) {
        $hashpassword = $row['AdminPassword']; // Lấy mật khẩu đã được băm từ cơ sở dữ liệu
        // Xác thực mật khẩu
